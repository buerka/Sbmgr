...
脚本权限 0700；程序、脚本、应用目录及父目录由 root 持有且不可被其他用户修改。提前核验主机指纹；客户端严格检查 known_hosts，不接受交互认证。固定入口忽略 `SSH_ORIGINAL_COMMAND`，只接受状态、只读落地目录 inventory、prepare/commit/rollback/finalize 和 usage/access；消息 ≤2 MiB，校验协议版本、集合、身份和角色，不接收任意命令或完整配置。目录仅返回 tag、名称、类型，不返回代理地址和凭据；旧从机不支持目录时提示升级，不改变已有线路。
...
- Web 画布按入口显示该服务器可用落地，拖线或点击连线会保存为单入口线路。保存与应用分开，未应用的新增线路不能分配。已有多跳线路可从详情编辑，参数化 CLI 继续支持完整多跳编排。
- 应用成功后，在用户列表或设备详情点击「分配线路」，勾选已保存线路，再「应用配置」；原有授权自动填入，其他设备不变。节点速率仍在设备节点详情修改。从机执行 `systemctl enable --now sbmgr` 开启维护。
