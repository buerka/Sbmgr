...
订阅交付或设备页按 `Enter` 打开菜单：`c` 通过 OSC52 请求复制完整 URL，`w` 保存到 `exports/` 的 0600 文本文件，`z` 显示二维码。不支持剪贴板确认的终端仅提示已发送请求；可改用保存文件。

交付前重新读取凭据并检查可用性。token 轮换立即撤销旧地址；默认界面隐藏 token，剪贴板、二维码和交付文件均属凭据。
...
| 设置变化 | 监听、证书、开关变化需重启 `sbmgr.service`，CUI 自动执行；token 撤销、用户禁用无需重启 |
