...
- Git 只管理源码、测试、文档、CI、构建和部署脚本。不得提交 `state.json`、真实 sing-box/Mihomo 配置、节点 YAML、UUID、订阅 token、密码、私钥、证书、导出、日志、备份或编译产物。
...
- `state.json` 是业务数据库。所有“读取—修改—保存”操作必须走跨进程状态锁、迁移、规范化和 `validateState`。
- 改动持久结构时递增 `stateVersion`，提供向前迁移，并增加旧状态迁移和新状态校验测试。
- 状态写入必须原子化且保持 `0600`。失败时不得留下部分更新。
