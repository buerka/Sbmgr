...
func TestRecentAccessArchiveAggregatesFiltersAndPrunes(t *testing.T) {
	...
	recordRecentAccess(&u, phone, "LAX", "api.openai.com", now)
	recordRecentAccess(&u, phone, "LAX", "api.openai.com", now.Add(time.Minute))
	recordRecentAccess(&u, &Device{Name: "pc"}, "ATT", "example.com", now.Add(2*time.Minute))
	...
	for query, target := range map[string]string{"openai": "api.openai.com", "phone": "api.openai.com", "att": "example.com"} {
...
func TestRecentAccessArchiveIsBounded(t *testing.T) {
	...
	for index := 0; index < recentAccessLimit+5; index++ {
		recordRecentAccess(&u, nil, "LAX", fmt.Sprintf("site-%04d.example", index), now.Add(time.Duration(index)*time.Second))
	}
...
func TestVersionSevenSeedsRecentAccessFromNodeStats(t *testing.T) {
	now := time.Now()
	s := &State{Version: 7, Users: []User{{Name: "alice", Nodes: []Node{{Name: "LAX", Device: "phone", Destinations: map[string]AccessStat{"example.com": {Count: 7, LastSeen: now.Format(time.RFC3339)}}}}}}}
...
func TestRecentAccessPageSupportsSearchAndSmallTerminal(t *testing.T) {
	...
	accesses := []RecentAccess{
		{Target: "api.openai.com", Device: "phone", Node: "LAX", FirstSeen: now.Format(time.RFC3339), LastSeen: now.Format(time.RFC3339), Count: 3},
		{Target: "example.com", Device: "pc", Node: "ATT", FirstSeen: now.Add(-time.Minute).Format(time.RFC3339), LastSeen: now.Add(-time.Minute).Format(time.RFC3339), Count: 1},
	}
	for index := 0; index < 12; index++ {
		seen := now.Add(-time.Duration(index+2) * time.Minute).Format(time.RFC3339)
		accesses = append(accesses, RecentAccess{Target: fmt.Sprintf("extra-%02d.example", index), Device: "phone", Node: "LAX", FirstSeen: seen, LastSeen: seen, Count: 1})
	}
	m := tuiModel{
	...
	page := m.render()
	if !strings.Contains(page, "api.openai.com") || !strings.Contains(page, "/ 搜索筛选") {
	...
	model, _ = m.updateAccessSearch(tea.KeyPressMsg(tea.Key{Text: "openai", Code: 'o'}))
	m = model.(tuiModel)
	if m.accessFilter != "openai" || len(recentAccessesForUser(findUser(m.state, "alice"), m.accessFilter)) != 1 {
