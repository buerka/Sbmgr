...
func TestCaptureAuditScreens(t *testing.T) {
	...
	for index, name := range []string{"alice", "bob", "carol", "dave", "erin", "frank"} {
		u := User{
			...
			Nodes: []Node{
				{Name: "LAX", Device: "默认设备", AuthUser: name + ":lax", UUID: "11111111-1111-4111-8111-111111111111", UploadMbps: 100, DownloadMbps: 300, Upload: 1 << 30, Download: 2 << 30, RateMark: rateMarkPrefix | uint32(index*3+1)},
				{Name: "ATT via LAX", Device: "默认设备", AuthUser: name + ":att", UUID: "22222222-2222-4222-8222-222222222222", Outbound: "to-att", UploadMbps: 25, DownloadMbps: 25, Upload: 700 << 20, Download: 900 << 20, RateMark: rateMarkPrefix | uint32(index*3+2)},
				{Name: "Frontier via LAX", Device: "默认设备", AuthUser: name + ":frontier", UUID: "33333333-3333-4333-8333-333333333333", Outbound: "to-frontier", UploadMbps: 25, DownloadMbps: 25, Upload: 90 << 20, Download: 160 << 20, RateMark: rateMarkPrefix | uint32(index*3+3)},
			},
		}
		if name == "frank" {
	...
	s := &State{
		...
		Users:  users, ReservedAuthUsers: []string{"LAX", "ATT via LAX", "Frontier via LAX"},
		...
		OutboundHealth: map[string]OutboundHealth{
			"to-att":      {Tag: "to-att", Target: "192.0.2.20:443", Healthy: true, LatencyMS: 7, CheckedAt: now.Format(time.RFC3339)},
			"to-frontier": {Tag: "to-frontier", Target: "203.0.113.20:443", Healthy: true, LatencyMS: 18, CheckedAt: now.Format(time.RFC3339)},
  ...
  "outbounds": [
    ...
    {"type":"shadowsocks","tag":"to-att","server":"192.0.2.20","server_port":443,"method":"chacha20-ietf-poly1305","password":"fixture-secret"},
    {"type":"shadowsocks","tag":"to-frontier","server":"203.0.113.20","server_port":443,"method":"chacha20-ietf-poly1305","password":"fixture-secret"}
  ],
  "route": {"final":"direct"}
}`
	...
	models := map[string]tuiModel{
		...
		"03-edit-form.ansi": {state: s, width: 100, height: 30, mode: tuiFormMode, status: "就绪", form: tuiForm{title: "出口健康与通知", active: 4, fields: []tuiField{
			...
			{label: "自定义目标", value: "to-att=192.0.2.20:443,to-frontier=203.0.113.20:443,backup=very-long-endpoint.example.com:443", cursor: 43, cursorSet: true},
