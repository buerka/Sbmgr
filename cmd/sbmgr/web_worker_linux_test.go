...
func TestWebPrivilegeWorkerTLSAuthenticationAndShutdown(t *testing.T) {
	...
	p, err := spawnWebProcess(ctx, executable, "127.0.0.1:0", webBootstrap{UID: 65534, GID: 65534, Origin: c.Origin, Cert: cert, Key: key}, b.lookup)
	if err != nil {
		t.Fatal(err)
	}
	defer p.stop()
	...
	if r, _ := call("/api/state", nil, nil); r.StatusCode != 401 {
		t.Fatal("unauthenticated worker access")
	}
	data, _ := json.Marshal(map[string]string{"username": "admin", "password": webTestPassword})
	r, _ := call("/api/login", data, nil)
	cookies := r.Cookies()
	if r.StatusCode != 200 || len(cookies) != 1 || !cookies[0].Secure {
		t.Fatal("worker login failed")
	}
	r, body := call("/api/state", nil, cookies[0])
